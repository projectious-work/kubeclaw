...
mkdocs build --strict            # Build site (strict mode catches broken links)
./scripts/deploy-docs.sh         # Deploy to pages branch (Codeberg Pages)
```
...
- **control-node.yaml.tpl** — Creates admin user, configures UFW (SSH from internal network + localhost for tunnel), fail2ban, SSH hardening. Uses `is_master` boolean: master installs cloudflared and allows localhost SSH; when `cloudflare_tunnel_token` is set, runs `cloudflared service install <token>` to auto-configure the tunnel as a systemd service. Replicas skip cloudflared sections (token always empty). When `enable_nat64`: configures DNS64 resolvers, NAT64 route, and networkd-dispatcher persistence. When `enable_k8s_prereqs`: installs selected `container_runtime` (containerd with SystemdCgroup, or CRI-O from OBS repo), kubeadm, kubelet, kubectl, kernel modules, sysctl params, disables swap, opens kubelet + etcd ports.
- **worker-node.yaml.tpl** — Similar but restrictive: no cloudflared, outbound limited to DNS/HTTP/S only, no TCP forwarding. When `enable_nat64`: uses DNS64 resolvers instead of Hetzner DNS, adds NAT64 prefix UFW rule. When `enable_k8s_prereqs`: same prerequisites as control node including `container_runtime` selection (no etcd ports).
...
- **deploy-docs.sh** — Builds MkDocs site and pushes to `pages` branch for Codeberg Pages deployment.
...
- Playbooks handle system updates, security hardening (unattended-upgrades, fail2ban, sysctl), NAT64/DNS64 configuration, and Kubernetes prerequisites (containerd or CRI-O, kubeadm, kubelet, kubectl)

### Documentation (`docs/`)
Documentation is built with MkDocs Material. Source files are in `docs/` with this structure:
- **docs/index.md** — Home page with project overview and architecture diagram
- **docs/getting-started/** — Requirements, quick start, dev container setup
- **docs/guide/** — Architecture, infrastructure (OpenTofu), manual setup, NAT64, Ansible, Kubernetes (kubeadm), OpenClaw deployment
- **docs/reference/** — Variables, outputs, cloud-init templates, playbooks, project structure, cost estimate
- **docs/operations/** — Troubleshooting, security, password management
- **docs/contributing/** — Contribution guide, code of conduct, development setup
...
- **Documentation**: All comments and documentation are in English. Documentation source is in `docs/` using MkDocs Material. Preview with `mkdocs serve`.
...
- kubeadm deployment guide (Cilium CNI, Hetzner CSI, namespace isolation, network policies)
- OpenClaw deployment guide (application-specific Cilium egress rules, StatefulSet)
- Dev Container with persistent SSH mount (.root/.ssh/)
- MkDocs documentation site with Material theme (24 pages across 6 sections)
