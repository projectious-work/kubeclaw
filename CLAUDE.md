...
```bash
./scripts/serve-docs.sh          # Preview docs at http://localhost:1313
./scripts/build-docs.sh          # Build the Hugo site locally
./scripts/deploy-docs.sh         # Deploy to gh-pages (GitHub Pages)
```
...
- **build-docs.sh** — Builds the Hugo + Docsy site locally.
- **serve-docs.sh** — Serves the Hugo site locally for preview.
- **deploy-docs.sh** — Builds Hugo and pushes the generated site to `gh-pages` for GitHub Pages deployment.
...
Documentation is built with Hugo + Docsy. Source files are in `content/`, with
the branded landing page at `content/_index.md` and the full documentation set
under `content/docs/`. Local search is generated with Docsy's offline Lunr
index. Release snapshots are published under version paths on `gh-pages`.
...
- **Documentation**: All comments and documentation are in English. Documentation source is in `content/` using Hugo + Docsy. Preview with `./scripts/serve-docs.sh`.
...
- Hugo documentation site with Docsy theme, offline search, version menu, and a projectious-branded landing page
