...
# 3. Create infrastructure (includes admin node by default)
tofu init
tofu apply
...
# 5. SSH to control node via admin node
ssh -J admin-node control-node
# Or use the ProxyJump config generated by setup-ssh.sh

# 6. Install Cloudflare Tunnel (on the control node)
sudo cloudflared service install <TOKEN>

# 7. Disable admin node and public IPv6 (after tunnel works)
