...
{{< alert title="cloudflared on both sides" >}}
...
{{< alert title="Save the token securely" >}}
...
{{< alert title="Authentication flow" >}}
...
{{< /alert >}}
After completing the Cloudflare setup, continue with [Infrastructure (OpenTofu)]({{< relref "/docs/guide/infrastructure" >}}) to configure the tunnel token and domain in `terraform.tfvars`, provision the cluster, and verify connectivity.
...
- [Infrastructure (OpenTofu)]({{< relref "/docs/guide/infrastructure" >}}) -- provision the cluster (if not done yet)
- [Server Management (Ansible)]({{< relref "/docs/guide/ansible" >}}) -- apply updates and hardening
