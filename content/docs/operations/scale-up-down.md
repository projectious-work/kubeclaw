...
4. Run Ansible playbooks on the new nodes -- see [Server Management (Ansible)]({{< relref "/docs/guide/ansible" >}}) for the full workflow.
...
After provisioning and running Ansible playbooks on new nodes, join them to the cluster. Follow [Step 2 of the Kubernetes guide]({{< relref "/docs/guide/kubernetes#step-2-join-worker-nodes-and-further-control-nodes" >}}) to run the `kubeadm join` command.
