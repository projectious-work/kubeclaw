...
{{< alert title="Note" >}}
The `lifecycle { ignore_changes = [user_data] }` block on all servers prevents re-provisioning when template variables change. Cloud-init only runs on first boot. Use [Ansible playbooks]({{< relref "/docs/reference/playbooks" >}}) for configuration changes on running nodes.
{{< /alert >}}
