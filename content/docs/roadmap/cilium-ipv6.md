...
{{< alert title="Implemented" >}}
This feature has been implemented. The [Kubernetes guide]({{< relref "/docs/guide/kubernetes" >}}) now uses dual-stack `kubeadm init` and Cilium with `ipv6.enabled=true` as the standard configuration.
...
- [DNS and NAT64 on KubeClaw]({{< relref "/docs/introduction/dns-and-nat64" >}})
