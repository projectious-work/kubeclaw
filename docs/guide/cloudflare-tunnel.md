...
!!! note "cloudflared on both sides"
...
After completing the Cloudflare setup, continue with [Infrastructure (OpenTofu)](infrastructure.md) to configure the tunnel token and domain in `terraform.tfvars`, provision the cluster, and verify connectivity.
...
- [Infrastructure (OpenTofu)](infrastructure.md) -- provision the cluster (if not done yet)
- [Server Management (Ansible)](ansible.md) -- apply updates and hardening
